...
-- ÿÿÿ reusing the model's prior choices and tool results ÿÿÿ without re-paying LLM
...
runToolCalls env spec toolCalls roundIx ensureStart
  -- ?6.1.3: a round mixing 'submit' with other calls is rejected wholesale ÿÿÿ no
...
runSubmitCall env spec roundIx callIx ensureStart tc = do
  ...
  case asSubmit spec of
    ...
    Just submit -> case validateSubmit (ssSchema submit) tc.tcArguments of
      Left reason ->
        -- ?6.1.3: a decode failure is recoverable ÿÿÿ the model can retry.
